...
fig_dpnl = go.Figure(
    go.Bar(x=x, y=y, text=y, marker_color=np.where(y >= 0, "deepskyblue", "crimson"))
)
...
fig_tpnl.add_trace(
    go.Scatter(x=daily_pnl.index.tolist(), y=daily_pnl["capital"]), secondary_y=False
)
fig_tpnl.add_trace(
    go.Scatter(
        x=daily_pnl.index.tolist(), y=daily_pnl["pct_pnl"], visible="legendonly"
    ),
    secondary_y=True,
)
...
fig_gain = px.bar(
    top_gain["Details"],
    top_gain["Realized Equity Change"],
    text=top_gain["Details"],
    title="Top Gainers",
    template="simple_white",
    color=top_gain["Realized Equity Change"],
...
fig_gain.update_traces(
    ...
    textposition="outside",
    hovertemplate="pnl: %{x} <br>symbol: %{text}",
    cliponaxis=False,
)
...
fig_loss = px.bar(
    top_loss["Details"],
    top_loss["Realized Equity Change"],
    text=top_loss["Details"],
    title="Top Losers",
    template="simple_white",
    color=top_loss["Realized Equity Change"],
...
# %%
fig_most_traded = px.pie(
    values=most_traded["counts"],
    names=most_traded["Details"],
    title="Most Traded Asset",
    hole=0.3,
)
fig_most_traded.update_layout(
    showlegend=True,
    title_x=0.5,
    plot_bgcolor="rgba(0,0,0,0)",
    paper_bgcolor="rgba(0,0,0,0)",
...
app.layout = html.Div(
    children=[
        ...
        html.Div(
            [
                html.Div([dcc.Graph(figure=fig_loss)], className="six columns"),
                html.Div([dcc.Graph(figure=fig_gain)], className="six columns"),
            ],
            className="row",
        ),
        html.Div(
            [
                html.Div([dcc.Graph(figure=fig_most_traded)], className="six columns"),
                html.Div([dcc.Graph(figure=fig_stat)], className="six columns"),
            ],
            className="row",
        ),
        # html.Div([
